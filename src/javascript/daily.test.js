...
test("AB/BC filter, callback", ()=>{
    let newArr = [];
    const peopleSmall = [{fname:"Alex", lname:"Smith", province:"BC", age:33}, {fname:"Angela", lname:"Jones", province:"AB", age:61}];
    let cb = (s) => { newArr.push("Callback on: " + s.fname); } 
    daily.filterAlbertaAndBCResidents(peopleSmall, cb);
    expect(newArr).toStrictEqual(["Callback on: Alex", "Callback on: Angela"]);

    const peopleSmall2 = [{fname:"Alex", lname:"Smith", province:"BC", age:33}, {fname:"Angela", lname:"Jones", province:"SK", age:61}];
    newArr = [];
    daily.filterAlbertaAndBCResidents(peopleSmall2, cb);
    expect(newArr).toStrictEqual(["Callback on: Alex"]);

    const expectedResidents = ["Alex Smith","Angela Jones","Byron Cardenas","Carrie Ramirez","Dustin Bullock","Eva Keiths","Forest Vaughn","Giovanni Browning","Ian Fitzgerald","James Kramer","Jinbong Robinson","Jingnan Hatfield","Latora Matthews","Lauren McClure","Luis Price","Marcela Perez","Monica Giles","Niloufar Carson","Roman Swanson","Seun Kelly","Thomas Hart","Trent Kerr"];
    expect(daily.filterAlbertaAndBCResidents(people, daily.getFirstLastNames)).toStrictEqual(expectedResidents);
});
